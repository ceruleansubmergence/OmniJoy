...
#define RESET_PIN RPI_GPIO_P1_15
...
//  for (i = 0; i < 320; i++) {
//wiringPiSPIDataRW(channel,cmd,len - 1);
    bcm2835_spi_writenb(data, 480*320*3);
//    data+=480*3;
 // }
...
void send_command(unsigned char *cmd, int len) {
  ...
  cmd++;
  if (len > 1) bcm2835_spi_writenb(cmd, len - 1);
}
...
int init_Display(void) {
  ...
  if (!bcm2835_init()) {
    syslog(LOG_ERR, "Unable to connect to init bcm2835\n");
  } else {
    syslog(LOG_ERR, "Initialized bcm2835\n");
  }
  ...
  if (!bcm2835_spi_begin()) {
    syslog(LOG_ERR, "Unable to connect to bcm2835 SPI - Run as root?");
    return -1;
  } else {
